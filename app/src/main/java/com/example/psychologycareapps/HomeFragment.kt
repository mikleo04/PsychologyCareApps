...
class HomeFragment : Fragment() {
    ...
    fun initBarchart(dx: ArrayList<Float>, chart:ColumnChartView, tittle: String) {
        val columns = arrayListOf<Column>()
        for(i in 0 until dx.size) {
            val values = ArrayList<SubcolumnValue>()
            values.add(SubcolumnValue(dx[i], requireActivity().getColor(R.color.black)))
            val c =  Column(values)
            c.setHasLabels(true)
            columns.add(c)
        }
        ...
        axisY.name = "Score"
        axisX.name = tittle
        axisY.textColor = requireActivity().getColor(R.color.black)
        axisX.textColor = requireActivity().getColor(R.color.black)
        axisX.values = valX
    ...
    fun fetchMood() {
        ...
        refMood.get().addOnSuccessListener {documents ->
            ...
            initBarchart(moodData, requireActivity().findViewById(R.id.cc_mood), "Tanggal ke")
